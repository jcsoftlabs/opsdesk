...
export default async function WeeklyReportPage({
  ...
  const [transactions, closedSessions, referenceRate] = await Promise.all([
    prisma.transaction.findMany({
      where: { bureauId, createdAt: { gte: weekStart, lt: weekEnd } },
      include: { createdBy: { select: { fullName: true } }, client: { select: { fullName: true } } },
      orderBy: { createdAt: "asc" },
    }),
    prisma.cashSession.findMany({
      where: { bureauId, status: "CLOSED", closedAt: { gte: weekStart, lt: weekEnd } },
    }),
    getReferenceRateAt(user.organizationId, weekEnd),
  ]);
